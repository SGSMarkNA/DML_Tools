...
class TimeField(UI_Object.UI):
	...
	def __init__(self, name=None, **kwargs):
		...
		if name == None:
			name = cmds.timeField(**kwargs)
			super(TimeField, self).__init__(name, **dict(qtParent=parent))
			
		else:
			if cmds.timeField(name, exists=True):
				super(TimeField, self).__init__(name)
			else:
				name = cmds.timeField(name, **kwargs)
				super(TimeField, self).__init__(name, **dict(qtParent=parent))
	#----------------------------------------------------------------------
