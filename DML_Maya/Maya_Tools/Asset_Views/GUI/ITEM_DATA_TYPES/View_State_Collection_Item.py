...
if False:
	from ...Custom_Nodes.DML_Asset_Views_View_State_Collection import Asset_Views_View_State_Collection


########################################################################
class View_State_Collection_Item_Data(Maya_Node_Item.Maya_Node_Item_Data):
	#----------------------------------------------------------------------
	def __init__(self,maya_node,**kwargs):
		...
		"""
		super(View_State_Collection_Item_Data, self).__init__(maya_node,**kwargs)
		
		if False:
			self.internal_data = Asset_Views_View_State_Collection()
	##----------------------------------------------------------------------
	#def _fn_get_display_name_value(self):
		#return self.internal_data.collection_name
	##----------------------------------------------------------------------
	#def _fn_set_display_name_value(self, value):
		...
		#self.internal_data.collection_name = value
...
########################################################################
class On_View_State_Added_Or_Removed_Callback(object):
	""""""
	def __init__(self,model_item):
		"""Constructor"""
		# Used To Check If The Action Needed To Taken Has Allready Been Set To Run
		...
		# a membership changes only once after all modifcation in maya are done
		self.qued = False
		isinstance(model_item,View_State_Collection_Data_Model_Item)
		self.main_window      = model_item.model.parentWidget()
		self.model_item       = model_item
	...
	def __call__(self,msg,plug,otherPlug,clientData):
		...
		"""
		if plug.partialName() == "viewStates":
			
			if Node_Messages.Was_Connection_Made(msg):
				fn  = OM.MFnDependencyNode(otherPlug.node())
				print("View State {} Was Added To {}".format(fn.name(),clientData.name))
				...
				if child_tree_item is None:
					...
					View_State_Item.View_State_Data_Model_Item(dml_node,model=self.model_item.model,parent_item=self.model_item)
				
			elif Node_Messages.Was_Connection_Broken(msg):
				fn  = OM.MFnDependencyNode(otherPlug.node())
				print("View State {} Was Removed From {}".format(fn.name(),clientData.name))
				child_tree_item = self.model_item.find_child(fn.uuid().asString(), role=self.model_item.ID_ROLE.INTERNAL_ID)
...
########################################################################
class View_State_Collection_Data_Model_Item(Maya_Node_Item.Maya_Node_Data_Model_Item):
	""""""
	#----------------------------------------------------------------------
	def __init__(self,internal_item,**kwargs):
		...
		------------------------------------
			...
			| **foreground_color** : *GlobalColor* : None
		"""
		internal_item = View_State_Collection_Item_Data(internal_item)
		super(View_State_Collection_Data_Model_Item,self).__init__(internal_item,**kwargs)
	...
	def Register_Node_Callbacks(self):
		...
		cbs = []
		cbs.append(DML_Tools.DML_Maya.Maya_Utils.API_Callback_Builders.create_Attribute_Changed_Callback(maya_node.apiMObject, On_View_State_Added_Or_Removed_Callback(self), clientData=maya_node))
		return cbs
	#----------------------------------------------------------------------
	def rebuild_Children(self):
		""""""
		super(View_State_Collection_Data_Model_Item,self).rebuild_Children()
		for node in self.get_Maya_Node().get_View_States():
			child = View_State_Item.View_State_Data_Model_Item(node,model=self.model,parent_item=self)
	#----------------------------------------------------------------------
	def get_Maya_Node(self):
		"""Returns The DML_Maya_Node assigned to this model item"""
		data = self.get_internal_Data(0)
		if False:
			isinstance(data,Asset_Views_View_State_Collection)
		return data
