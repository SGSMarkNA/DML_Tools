...
class Node_Return_Type_Publication(type):
	...
	def __new__(mcs, name, bases, dct):
		...
		MAYA_NODE_TYPE_RELATION   = dct.pop('MAYA_NODE_TYPE_RELATION',None)
		RETURN_OVERIDE_CHECK_TYPE = dct.pop('RETURN_OVERIDE_CHECK_TYPE',None)
		
		res = type.__new__(mcs, name, bases, dct)
		
		if MAYA_NODE_TYPE_RELATION is not None:
			Data_Storage.Node_Return_Type_Relations[MAYA_NODE_TYPE_RELATION]=res
			
		if RETURN_OVERIDE_CHECK_TYPE is not None:
			if not Data_Storage.Node_Return_Type_Overides.has_key(RETURN_OVERIDE_CHECK_TYPE):
	...
	#def __call__(cls,*args,**kwargs):
		#return super(Node_Return_Type_Publication, cls).__call__(*args,**kwargs)
