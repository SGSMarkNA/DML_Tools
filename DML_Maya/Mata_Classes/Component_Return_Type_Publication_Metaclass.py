...
########################################################################
class Component_Return_Type_Publication(type):
	""""""
	#----------------------------------------------------------------------
	def __new__(mcs, name, bases, dct):
		...
		MAYA_COMPONENT_TYPE_RELATION   = dct.pop('MAYA_COMPONENT_TYPE_RELATION',None)
		RETURN_OVERIDE_CHECK_TYPE = dct.pop('RETURN_OVERIDE_CHECK_TYPE',None)
		
		res = type.__new__(mcs, name, bases, dct)
		
		if MAYA_COMPONENT_TYPE_RELATION is not None:
		...
		if RETURN_OVERIDE_CHECK_TYPE is not None:
			...
			if not res in Data_Storage.Component_Return_Type_Overides[RETURN_OVERIDE_CHECK_TYPE]:
				Data_Storage.Component_Return_Type_Overides[RETURN_OVERIDE_CHECK_TYPE].append(res)
		return res
